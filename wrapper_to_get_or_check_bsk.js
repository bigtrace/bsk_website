...
function bsk_solver(tbs_str) {
	...
	let IN = { tbs: tbs_str }; // this is tbs
	...
	var funcs = [
		...
		[
			'w4',
			function() {
				return '0,1,2,window,self,document,name,location,customElements,history,locationbar,menubar,personalbar,scrollbars,statusbar,toolbar,status,closed,frames,length,top';
			}
		],
		[
			't2',
		...
		[
			'm2',
			function() {
				return `(${Math.floor(Math.random() * 1000) + 400},0),(428,443),(${Math.floor(Math.random() * 1000) +
					400},422),(${Math.floor(Math.random() * 1000) + 400},366),(${Math.floor(Math.random() * 1000) +
					400},1),(${Math.floor(Math.random() * 1000) + 400},279),(510,3),(${Math.floor(
					Math.random() * 1000
				) + 400},1377),(1029,1453),(834,1680)`; //_BSK.userBehavior.mouseMoveArr.join() || 'NULL'
			}
		],
		[
			'm3',
		...
		[
			'm4',
			function() {
				return `900,970,${Math.floor(Math.random() * 1500) + 1000},${Math.floor(Math.random() * 2000) + 3000}`; //_BSK.userBehavior.scrollArr.join() || 'NULL'
			}
		],
		[
			'm5',
		...
		[
			'k2',
			function() {
				return `${Math.floor(Math.random() * 20) + 60},229,${Math.floor(Math.random() * 20) +
					60},229,${Math.floor(Math.random() * 20) + 60},229,${Math.floor(Math.random() * 20) +
					60},229,65,229`; //_BSK.userBehavior.keyUpArr.join() || 'NULL'
			}
		]
	];
	startRun();
...
function decode_bsk(tbs_str, bsk_to_decrypt) {
	function decodeCharCode(code) {
		return ((6 ^ code) - 3) ^ 5;
	}
	...
	const map_static = toCharCodeArr(tbs_str);
...
module.exports.bsk_solver = bsk_solver;
module.exports.decode_bsk = decode_bsk;
